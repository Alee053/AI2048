...
struct TTEntry {
    ...
    uint8_t  occupied; // Explicitly distinguishes a valid zero key from an empty slot.
};

static_assert(sizeof(TTEntry) == 16, "TTEntry must remain cache-line dense");
...
public:
    ...
    TranspositionTable() {
        // Allocate once. Zero-initialise so occupied==0 means "empty slot".
    ...
    static uint64_t hash_key(uint64_t key, uint8_t type, uint8_t depth) {
        uint64_t x = key ^ (static_cast<uint64_t>(type) << 1) ^ (static_cast<uint64_t>(depth) << 8);
    ...
    bool probe(uint64_t key, uint8_t depth, NodeType type, float& out_score) const {
        uint32_t idx = static_cast<uint32_t>(
            hash_key(key, static_cast<uint8_t>(type), depth) & BUCKET_MASK
        );
        const TTBucket& bucket = table[idx];
        
        for (int i = 0; i < 4; ++i) {
            const TTEntry& entry = bucket.entries[i];
            if (entry.occupied && entry.key == key && entry.type == static_cast<uint8_t>(type) &&
                entry.depth == depth) {
    ...
    void store(uint64_t key, uint8_t depth, NodeType type, float score) {
        ...
        // 1. If this exact state/depth entry already exists, overwrite it.
        for (int i = 0; i < 4; ++i) {
            if (bucket.entries[i].occupied && bucket.entries[i].key == key &&
                bucket.entries[i].type == static_cast<uint8_t>(type) && bucket.entries[i].depth == depth) {
        ...
        // 3. All slots full: preserve deeper entries globally. Generation age
        // only breaks ties between entries at the same minimum depth.
        collision_count_++;
        int replace_idx = static_cast<int>(collision_count_ % 4);
        uint8_t min_depth = bucket.entries[replace_idx].depth;
        uint8_t best_age = 0;
        for (int offset = 0; offset < 4; ++offset) {
            int i = static_cast<int>((collision_count_ + offset) % 4);
            uint8_t age = static_cast<uint8_t>(
                (current_generation_ - bucket.entries[i].generation) & 0x1F
            );
            if (bucket.entries[i].depth < min_depth ||
                (bucket.entries[i].depth == min_depth && age > best_age)) {
                replace_idx = i;
        ...
        bucket.entries[replace_idx].occupied = 1;
    }
